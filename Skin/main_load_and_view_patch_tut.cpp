...
//    }
    ...
    if(!fi.absoluteDir().exists())  //Returns the file's absolute path as a QDir object
    {
        qCritical("Invalid path '%s'",configFilePath.toAscii().data());
        return -1;
    }

    if(!fi.exists())  //Returns true if the file exists; otherwise returns false
    {
    ...
    if(fi.isDir() || fileDiagEnabled == true)  //Returns true if this object points to a directory or to a symbolic link to a            directory; otherwise returns false
    {
