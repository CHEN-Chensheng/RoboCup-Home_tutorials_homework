...
void TurtleClass::getPose(const turtle_vis::DesiredPose::ConstPtr &msg)
{
    ...
    ROS_INFO("sending back response: x = %f, y = %f, theta = %f",  turtlePose_g[0], turtlePose_g[1], turtlePose_g[2]);
...
bool TurtleClass::getDPose(turtle_vis::send_desired_pose::Request &req, turtle_vis::send_desired_pose::Response &res)
{
    ...
    pthread_mutex_lock( &this->count_mutex );  //线程管理
    ...
    ROS_INFO("sending back response: x = %f, y = %f, theta = %f, reply = %d", req.x, req.y, req.theta, res.reply);
