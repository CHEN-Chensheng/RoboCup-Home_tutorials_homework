...
#include <tf/transform_broadcaster.h>  //ros下使用tf就必须得有broadcaster头文件
...
int main( int argc, char** argv )
{
    ...
    static tf::TransformBroadcaster br;  //都有的申明
   ...
   ros::Subscriber sub=n.subscribe("turtle_control",100,
    ...
    while(ros::ok())
    {
        ...
        br.sendTransform(tf::StampedTransform(transform,ros::Time::now(),"/world","/turtle")); //这边的/turtle需要和上面的createMarkerMesh里面的turtle名字一致，rviz中才会出现乌龟
